...
Object.keys(person31)
    ['age']

//В массив значений:
Object.values(person31)
    [31]
*/

if (!person.name) {    // !person.name - вернет undefined. !undefined === true  > true, следовательно выполняется блок кода
    console.log("Имя не указано")
    // Другое действие в случае, если свойства "name" у объекта "person" нету
}
...
//Тернарный оператор - у него 3 операнда
// Конструкция с тернарным оператором - это ВЫРАЖЕННИЕ (выражение всегда возвращает значение)
...
const ichArray = [ true, "abc" , 10]

/// for (Начальная инструкция; Условие; Итерационное действие) {
//// Блок кода, выполняемый на каждой итерации
///}

for (let i = 0; i < ichArray.length; i++) {
    console.log(ichArray[i])
}

///// while (Условие) {
// Блок кода, выполняемый на каждой итерации
// Цикл выполняется пока условие правдиво, если ложно, то произойдет мгновенный выход из цикла
// }

let i = 0
while (i<5) {
    console.log(i)
    i++ // изменяем переменную, которая меняет условие. (без нее цикл будет бесконечным)
}

///// do...while
// Блок кода выполняется хотя бы 1 раз
...
for (const key in ichArray) {
    console.log(key, ichArray[key])
}

////// for...of...

const myString = "Zaytsev"

for (const myStringElement of myString) {
    console.log(myStringElement)
}

for (const ichArrayElement of ichArray) {
    console.log(ichArrayElement)
}
